...
if ( !function_exists( 'charity_is_hope_sc_gap_reg_shortcodes' ) ) {
	...
	function charity_is_hope_sc_gap_reg_shortcodes() {
		...
		charity_is_hope_sc_map("trx_gap", array(
			"title" => esc_html__("Gap", 'charity-is-hope'),
			"desc" => wp_kses_data( __("Insert gap (fullwidth area) in the post content. Attention! Use the gap only in the posts (pages) without left or right sidebar", 'charity-is-hope') ),
			"decorate" => true,
			"container" => true,
			"params" => array(
				"_content_" => array(
					"title" => esc_html__("Gap content", 'charity-is-hope'),
					"desc" => wp_kses_data( __("Gap inner content", 'charity-is-hope') ),
					"rows" => 4,
					"value" => "",
					"type" => "textarea"
				)
...
if ( !function_exists( 'charity_is_hope_sc_gap_reg_shortcodes_vc' ) ) {
	...
	function charity_is_hope_sc_gap_reg_shortcodes_vc() {
		...
		vc_map( array(
			"base" => "trx_gap",
			"name" => esc_html__("Gap", 'charity-is-hope'),
			"description" => wp_kses_data( __("Insert gap (fullwidth area) in the post content", 'charity-is-hope') ),
