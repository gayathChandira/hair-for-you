...
class charity_is_hope_widget_calendar extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'widget_calendar', 'description' => esc_html__('Calendar for posts and/or Events', 'charity-is-hope'));
		parent::__construct( 'charity_is_hope_widget_calendar', esc_html__('Charity Is Hope - Advanced Calendar', 'charity-is-hope'), $widget_ops );
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>_1"><?php esc_html_e('Post type:', 'charity-is-hope'); ?></label><br>
			<?php
				$i=0;
			...
			<br><span class="description"><?php esc_html_e('Attention! If you check custom post types, please check also settings in the correspond plugins and enable display this posts in the blog!', 'charity-is-hope'); ?></span>
		</p>
...
if (!function_exists('charity_is_hope_widget_calendar_reg_shortcodes_vc')) {
	...
	function charity_is_hope_widget_calendar_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Calendar", 'charity-is-hope'),
				"description" => wp_kses_data( __("Insert standard WP Calendar, but allow user select week day's captions", 'charity-is-hope') ),
				...
				"params" => array(
					...
					array(
						"param_name" => "weekdays",
						"heading" => esc_html__("Week days", 'charity-is-hope'),
						"description" => wp_kses_data( __("Show captions for the week days as three letters (Sun, Mon, etc.) or as one initial letter (S, M, etc.)", 'charity-is-hope') ),
						"class" => "",
						"value" => array(esc_html__("Initial letter", 'charity-is-hope') => "initial" ),
						"type" => "checkbox"
					),
