...
if ( !function_exists( 'charity_is_hope_core_theme_setup' ) ) {
	...
	function charity_is_hope_core_theme_setup() {
		...
		add_action('wp_ajax_nopriv_frontend_editor_save', 'charity_is_hope_callback_frontend_editor_save');

		// Frontend editor: Delete post
		add_action('wp_ajax_frontend_editor_delete', 	'charity_is_hope_callback_frontend_editor_delete');
		add_action('wp_ajax_nopriv_frontend_editor_delete', 'charity_is_hope_callback_frontend_editor_delete');
...
if ( !function_exists( 'charity_is_hope_core_frontend_scripts' ) ) {
	function charity_is_hope_core_frontend_scripts() {
		...
		wp_enqueue_script( 'charity_is_hope-core-modernizr-script', charity_is_hope_get_file_url('js/photostack/modernizr.min.js'), array(), null, false );
	    ...
	    if ( 'off' !== _x( 'on', 'Google fonts: on or off', 'charity-is-hope' ) ) {
			...
			if (is_array($theme_fonts) && count($theme_fonts) > 0) {
				...
				foreach ($theme_fonts as $font=>$v) {
					if (isset($fonts[$font])) {
						...
						if (!empty($fonts[$font]['css'])) {
							$css = $fonts[$font]['css'];
							wp_enqueue_style( 'charity_is_hope-font-'.str_replace(' ', '-', $font_name).'-style', $css, array(), null );
						} else {
				...
				if ($google_fonts) {
					wp_enqueue_style( 'charity_is_hope-font-google_fonts-style', add_query_arg( 'family', urlencode( $google_fonts.'&subset='.charity_is_hope_get_theme_option('fonts_subset') ), "//fonts.googleapis.com/css" ), array(), null );
				}
			}
		}
		...
		// Main stylesheet
		wp_enqueue_style( 'charity_is_hope-main-style', get_stylesheet_uri(), array(), null );
		...
		if (charity_is_hope_get_theme_option('css_animation')=='yes' && (charity_is_hope_get_theme_option('animation_on_mobile')=='yes' || !wp_is_mobile()) && !charity_is_hope_vc_is_frontend())
			wp_enqueue_style( 'charity_is_hope-animation-style',	charity_is_hope_get_file_url('css/core.animation.css'), array(), null );
		...
		if (charity_is_hope_get_theme_option('responsive_layouts') == 'yes') {
			$suffix = '';
			wp_enqueue_style( 'charity_is_hope-responsive-style', charity_is_hope_get_file_url('css/responsive'.($suffix).'.css'), array(), null );
			do_action('charity_is_hope_action_add_responsive');
			$css = apply_filters('charity_is_hope_filter_add_responsive_inline', '');
			if (!empty($css)) wp_add_inline_style( 'charity_is_hope-responsive-style', $css );
		}
		...
		if (in_array(charity_is_hope_get_theme_option('menu_hover'), array('slide_line', 'slide_box'))) {
			wp_enqueue_script( 'charity_is_hope-slidemenu-script', charity_is_hope_get_file_url('js/jquery.slidemenu.js'), array('jquery'), null, true );
		}

		if ( is_single() && charity_is_hope_get_custom_option('show_reviews')=='yes' ) {
			wp_enqueue_script( 'charity_is_hope-core-reviews-script', charity_is_hope_get_file_url('js/core.reviews.js'), array('jquery'), null, true );
		}

		wp_enqueue_script( 'charity_is_hope-core-utils-script',	charity_is_hope_get_file_url('js/core.utils.js'), array('jquery'), null, true );
		wp_enqueue_script( 'charity_is_hope-core-init-script',	charity_is_hope_get_file_url('js/core.init.js'), array('jquery'), null, true );
		wp_enqueue_script( 'charity_is_hope-theme-init-script',	charity_is_hope_get_file_url('js/theme.init.js'), array('jquery'), null, true );
		...
		if (charity_is_hope_get_custom_option('show_video_bg') == 'yes' && charity_is_hope_get_custom_option('video_bg_youtube_code') != '') {
			wp_enqueue_script( 'charity_is_hope-video-bg-script', charity_is_hope_get_file_url('js/jquery.tubular.1.0.js'), array('jquery'), null, true );
		}
		...
		if (is_singular() && !charity_is_hope_storage_get('blog_streampage') && charity_is_hope_get_custom_option('show_share')!='hide') {
			wp_enqueue_script( 'charity_is_hope-social-share-script', charity_is_hope_get_file_url('js/social/social-share.js'), array('jquery'), null, true );
		}
		...
		if (charity_is_hope_get_theme_option('show_theme_customizer') == 'yes') {
			if (file_exists(charity_is_hope_get_file_dir('core/core.customizer/front.customizer.css')))
				wp_enqueue_style(  'charity_is_hope-customizer-style',  charity_is_hope_get_file_url('core/core.customizer/front.customizer.css'), array(), null );
			if (file_exists(charity_is_hope_get_file_dir('core/core.customizer/front.customizer.js')))
				wp_enqueue_script( 'charity_is_hope-customizer-script', charity_is_hope_get_file_url('core/core.customizer/front.customizer.js'), array(), null, true );
		}
		
		//Debug utils
		if (charity_is_hope_get_theme_option('debug_mode')=='yes') {
			wp_enqueue_script( 'charity_is_hope-core-debug-script', charity_is_hope_get_file_url('js/core.debug.js'), array(), null, true );
		}
...
if ( !function_exists( 'charity_is_hope_enqueue_slider' ) ) {
	function charity_is_hope_enqueue_slider($engine='all') {
		if ($engine=='all' || $engine=='swiper') {
			wp_enqueue_style(  'charity_is_hope-swiperslider-style', 			charity_is_hope_get_file_url('js/swiper/swiper.css'), array(), null );
			// jQuery version of Swiper conflict with Revolution Slider!!! Use DOM version
			wp_enqueue_script( 'charity_is_hope-swiperslider-script', 			charity_is_hope_get_file_url('js/swiper/swiper.js'), array(), null, true );
		}
	}
}
...
if ( !function_exists( 'charity_is_hope_enqueue_polaroid' ) ) {
	function charity_is_hope_enqueue_polaroid() {
		wp_enqueue_style(  'charity_is_hope-polaroid-style', 	charity_is_hope_get_file_url('js/photostack/component.css'), array(), null );
		wp_enqueue_script( 'charity_is_hope-classie-script',		charity_is_hope_get_file_url('js/photostack/classie.js'), array(), null, true );
		wp_enqueue_script( 'charity_is_hope-polaroid-script',	charity_is_hope_get_file_url('js/photostack/photostack.js'), array(), null, true );
	}
}
...
if ( !function_exists( 'charity_is_hope_enqueue_messages' ) ) {
	function charity_is_hope_enqueue_messages() {
		wp_enqueue_style(  'charity_is_hope-messages-style',		charity_is_hope_get_file_url('js/core.messages/core.messages.css'), array(), null );
		wp_enqueue_script( 'charity_is_hope-messages-script',	charity_is_hope_get_file_url('js/core.messages/core.messages.js'),  array('jquery'), null, true );
	}
}
...
if ( !function_exists( 'charity_is_hope_enqueue_portfolio' ) ) {
	function charity_is_hope_enqueue_portfolio($hover='') {
		wp_enqueue_style( 'charity_is_hope-portfolio-style',  charity_is_hope_get_file_url('css/core.portfolio.css'), array(), null );
...
if ( !function_exists( 'charity_is_hope_enqueue_diagram' ) ) {
	function charity_is_hope_enqueue_diagram($type='all') {
		if ($type=='all' || $type=='pie') wp_enqueue_script( 'charity_is_hope-diagram-chart-script',	charity_is_hope_get_file_url('js/diagram/chart.min.js'), array(), null, true );
		if ($type=='all' || $type=='arc') wp_enqueue_script( 'charity_is_hope-diagram-raphael-script',	charity_is_hope_get_file_url('js/diagram/diagram.raphael.min.js'), array(), 'no-compose', true );
	}
}
...
if ( !function_exists( 'charity_is_hope_enqueue_popup' ) ) {
	function charity_is_hope_enqueue_popup($engine='') {
		if ($engine=='pretty' || (empty($engine) && charity_is_hope_get_theme_option('popup_engine')=='pretty')) {
			wp_enqueue_style(  'charity_is_hope-prettyphoto-style',	charity_is_hope_get_file_url('js/prettyphoto/css/prettyPhoto.css'), array(), null );
			wp_enqueue_script( 'charity_is_hope-prettyphoto-script',	charity_is_hope_get_file_url('js/prettyphoto/jquery.prettyPhoto.min.js'), array('jquery'), 'no-compose', true );
		} else if ($engine=='magnific' || (empty($engine) && charity_is_hope_get_theme_option('popup_engine')=='magnific')) {
			wp_enqueue_style(  'charity_is_hope-magnific-style',	charity_is_hope_get_file_url('js/magnific/magnific-popup.css'), array(), null );
			wp_enqueue_script( 'charity_is_hope-magnific-script',charity_is_hope_get_file_url('js/magnific/jquery.magnific-popup.min.js'), array('jquery'), '', true );
...
if ( !function_exists( 'charity_is_hope_core_frontend_add_vars' ) ) {
	...
	function charity_is_hope_core_frontend_add_vars() {
		...
		if (!empty($vars)) wp_localize_script( 'charity_is_hope-core-init-script', 'CHARITY_IS_HOPE_STORAGE', $vars);
...
if (!function_exists('charity_is_hope_core_add_property_to_link')) {
	//add_filter('style_loader_tag', 'charity_is_hope_core_add_property_to_link', 10, 3);
...
if ( !function_exists( 'charity_is_hope_add_mainmenu_classes' ) ) {
	// add_filter('wp_nav_menu_objects', 'charity_is_hope_add_mainmenu_classes', 10, 2);
