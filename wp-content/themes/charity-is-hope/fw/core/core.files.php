...
if (!function_exists('charity_is_hope_esc')) {
	function charity_is_hope_esc($file) {
...
if (!function_exists('charity_is_hope_enqueue_style')) {	
	function charity_is_hope_enqueue_style($handle, $src=false, $depts=array(), $ver=null, $media='all') {
		$load = true;
		if (!is_array($src) && $src !== false && $src !== '') {
			$debug_mode = charity_is_hope_get_theme_option('debug_mode');
			$theme_dir = get_template_directory();
			$theme_url = get_template_directory_uri();
			$child_dir = get_stylesheet_directory();
			$child_url = get_stylesheet_directory_uri();
			$dir = $url = '';
			if (charity_is_hope_strpos($src, $child_url)===0) {
				$dir = $child_dir;
				$url = $child_url;
			} else if (charity_is_hope_strpos($src, $theme_url)===0) {
				$dir = $theme_dir;
				$url = $theme_url;
			}
			if ($dir != '') {
				if ($debug_mode == 'no') {
					if (charity_is_hope_substr($src, -4)=='.css') {
						if (charity_is_hope_substr($src, -8)!='.min.css') {
							$src_min = charity_is_hope_substr($src, 0, charity_is_hope_strlen($src)-4).'.min.css';
							$file_src = $dir . charity_is_hope_substr($src, charity_is_hope_strlen($url));
							$file_min = $dir . charity_is_hope_substr($src_min, charity_is_hope_strlen($url));
		...
		if ($load) {
			if (is_array($src))
				...
				wp_enqueue_style( $handle, esc_url($src).(charity_is_hope_param_is_on(charity_is_hope_get_theme_option('debug_mode')) ? (charity_is_hope_strpos($src, '?')!==false ? '&' : '?').'rnd='.mt_rand() : ''), $depts, $ver, $media );
		}
	}
}

// Enqueue .min.js (if exists and filetime .min.js > filetime .js) instead .js
if (!function_exists('charity_is_hope_enqueue_script')) {	
	function charity_is_hope_enqueue_script($handle, $src=false, $depts=array(), $ver=null, $in_footer=false) {
		$load = true;
		if (!is_array($src) && $src !== false && $src !== '') {
			$debug_mode = charity_is_hope_get_theme_option('debug_mode');
			$theme_dir = get_template_directory();
			...
			if ($dir != '') {
				if ($debug_mode == 'no') {
					if (charity_is_hope_substr($src, -3)=='.js') {
						if (charity_is_hope_substr($src, -7)!='.min.js') {
							$src_min  = charity_is_hope_substr($src, 0, charity_is_hope_strlen($src)-3).'.min.js';
							$file_src = $dir . charity_is_hope_substr($src, charity_is_hope_strlen($url));
							$file_min = $dir . charity_is_hope_substr($src_min, charity_is_hope_strlen($url));
							if (file_exists($file_min) && filemtime($file_src) <= filemtime($file_min)) $src = $src_min;
						}
		...
		if ($load) {
			if (is_array($src))
				...
				wp_enqueue_script( $handle, esc_url($src).(charity_is_hope_param_is_on(charity_is_hope_get_theme_option('debug_mode')) ? (charity_is_hope_strpos($src, '?')!==false ? '&' : '?').'rnd='.mt_rand() : ''), $depts, $ver, $in_footer );
		}
	}
}
...
if (!function_exists('charity_is_hope_get_socials_dir')) {	
	function charity_is_hope_get_socials_dir($soc, $return_url=false) {
		return charity_is_hope_get_file_dir('images/socials/' . charity_is_hope_esc($soc) . (charity_is_hope_strpos($soc, '.')===false ? '.png' : ''), $return_url, true);
	}
}
...
if (!function_exists('charity_is_hope_get_template_dir')) {	
	function charity_is_hope_get_template_dir($tpl) {
		return charity_is_hope_get_file_dir('templates/' . charity_is_hope_esc($tpl) . (charity_is_hope_strpos($tpl, '.php')===false ? '.php' : ''));
	}
}
?>